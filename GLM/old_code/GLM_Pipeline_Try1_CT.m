...
is_mac = 1; %For loading the data %2022-08-29 Use this to replace is_ron
is_ron = 0; %For setting path
...
for s = S_list %Do analyses for each session put in S_list
    ...
    for br = 1:length(BRs)
        ...
        %       12. brain_label = specify to which brain area each label
        %       belongs to
    ...
    % IMPORTANT NOTE: we're only getting the unique behaviors for now
    % behaviors that happen simultaneously are not included)
    ...
    %CT Note: keeping proximity for now
%     % Set proximity as rest
%     behavior_labels_subject(behavior_labels_subject==find(behav_categ=="Proximity"))=length(behav_categ); %exclude proximity for now (i.e. mark as "undefined").
%     behavior_labels_partner(behavior_labels_partner==find(behav_categ=="Proximity"))=length(behav_categ); %exclude proximity for now (i.e. mark as "undefined").
    ...
    obs_statevents_subject = unique(cell2mat(Behavs(:,1)));
...
% % %     obs_statevents_subject( obs_statevents_subject == prox_ind) = []; %Remove proximity as an observed event
    ...
    obs_statevents_partner = unique(cell2mat(Behavs(:,2)));

    %CT Note: for now we're removing regressors that are causing issues of
    %co-linearity, we might not need to do this once regularization is set up. 
    ...
    % Design matrix for the 4 variable groups: Subject; Partner; Context; Kinematics
    ...
    %CT note: to add delays change 0 to 3.
