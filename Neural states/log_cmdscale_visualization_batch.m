...
for s =session_range %1:length(sessions)
    ...
    for channel_flag = ["vlPFC", "TEO"]
        ...
        %behav =[4,5,6,9,10] ;%unique(behavior_labels); %[4,5,7,8,9,10,24];% [4:10, 23]; %[4:8,17]; %manually select behaviors of interest
        ...
        %run cmdscale
        D = squareform(pdist(zscore(Spike_count_raster_final)));
        cmdscale_plot_final = cmdscale(D);
        ...
        ax1=subplot(1,2,1);
        scatter3(cmdscale_plot_final(:,1), cmdscale_plot_final(:,2),cmdscale_plot_final(:,3),14,Cmap(behavior_labels_final,:),'filled')
        xlabel('mdscale 1'); ylabel('mdscale 2'); zlabel('mdscale 3'); 
        ...
        ax2=subplot(1,2,2);
        scatter3(cmdscale_plot_final(:,1), cmdscale_plot_final(:,2),cmdscale_plot_final(:,3),14,Cmap_block(block_labels_final,:),'filled')
        xlabel('mdscale 1'); ylabel('mdscale 2'); zlabel('mdscale 3'); 
        %set(gca,'xtick',[]); set(gca,'ytick',[]); set(gca,'ztick',[])
        title('Block')
        set(gca,'FontSize',12);
