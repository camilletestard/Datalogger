...
% % %         top_view_ME = [0; top_view_ME]; side_view_ME = [0; side_view_ME];
        ...
        %lag movement matrix
        lags= 0%[-1000 :100: 1000];
        for l=1:length(lags)
            
            mvmt_final_lagged = lagmatrix(mvmt_final,lags(l));


            for unit = 1:50%size(Y_final ,2) %for now one unit at a time.

                %unroll mvmt predictors (format for the multilinear regression)
                toplogger_x = mvmt_final_lagged(:,1);
                toplogger_y = mvmt_final_lagged(:,2);
                bottomlogger_x = mvmt_final_lagged(:,3);
                bottomlogger_y = mvmt_final_lagged(:,4);
                head_orientation_dlc = mvmt_final_lagged(:,5);
                dist_traveled = mvmt_final_lagged(:,6);
                acceleration = mvmt_final_lagged(:,7);

                %Set up predictor matrix
                X_all = table(toplogger_x, toplogger_y,...
                    bottomlogger_x, bottomlogger_y, head_orientation_dlc,...
                    dist_traveled, acceleration, Y_final(:,unit));

                X_all=X_all(randperm(size(X_all,1)),:);

                %Run model for all predictors
                mdl_all = fitlm(X_all); %run linear model with all predictors for specific unit
                Adj_rsq_all{s,l}(unit) = mdl_all.Rsquared.Adjusted; %extract adjusted Rsq
...
%                         Full_rsq_perReg{s,l}(unit, pred) = mdl.Rsquared.Adjusted; %extract adjusted Rsq
% 
...
%                         Unq_rsq_perReg{s,l}(unit, pred) = Adj_rsq_all{s}(unit) - mdl.Rsquared.Adjusted; %extract adjusted Rsq
% 
%                     end
%                 end
% 
...
% 
        ...
        clear avg_full_fit_allRegs
        for l=1:length(lags)
            avg_full_fit_allRegs(l) = mean(Adj_rsq_all{s,l});
%             avg_full_fit(l,:)=mean(Full_rsq_perReg{s,l});
%             sd_full_fit(l,:)=std(Full_rsq_perReg{s,l});
%             avg_unq_fit(l,:)=mean(Unq_rsq_perReg{s,l});
        end
%         figure; hold on
%         plot(avg_full_fit(:,2:end))
%         %xticks(''); xticklabels(string(lags))
%         xline(16,'LineStyle','--')
%         xlabel('Frames')

        [~, idx]=max(avg_full_fit_allRegs)

        figure; hold on
        plot(avg_full_fit_allRegs)
        xline(round(length(lags)/2),'LineStyle','--')
        xlabel('Frames')
