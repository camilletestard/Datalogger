...
mean_cohend_per_behav = nan(length(sessions), 27);
std_cohend_per_behav = nan(length(sessions), 27);
se_cohend_per_behav = nan(length(sessions), 27);
prop_selective_per_behav = nan(length(sessions), 27);
...
%     saveas(gcf, [savePath '/Baseline_epochs.png']); close all
    ...
    idx = find(behavior_labels == 7);
    unit=167;
    figure; hold on; set(gcf,'Position',[150 250 1500 500]);
    plot(1:session_length, Spike_raster_smooth(unit,1:session_length))
    plot(idx, Spike_raster_smooth(unit,idx), "LineWidth",2)
    mean(Spike_raster_smooth(unit,idx))
    plot(idx_rest, Spike_raster_smooth(unit,idx_rest), "LineWidth",1)
    mean(Spike_raster_smooth(unit,idx_rest))
    yline(1)
