...
for s =session_range %1:length(sessions)
   ...
   [Spike_rasters, labels, labels_partner, labels_neighbor, behav_categ, block_times, monkey, reciprocal_set, social_set, ME_final,unit_count, groom_labels_all]= log_GenerateDataToRes_function_neighbor(filePath, temp_resolution, channel_flag, is_mac, with_NC, isolatedOnly);
    ...
    behav = [4,5,18,7]; %focus on a set of behaviors which happen enough time in both the subject and neighbor and are not reciprocal
...
%     figure; hold on; subplot(2,1,1); hist(behavior_labels_neighbor, 30); title('Neighbor behavior'); subplot(2,1,2); hist(behavior_labels_subject_during_neighbor_behav, 30); title('Subject behavior during neighbor idx')
%     figure; hold on; subplot(2,1,1); hist(behavior_labels_subject, 30); title('Subject behavior'); subplot(2,1,2); hist(behavior_labels_neighbor_during_subject_behav, 30); title('Neighbor behavior during subject idx')
    ...
    if plot_toggle
        ...
        subplot(1,4,3); hp=heatmap([cohend_thresh(:,:,1)-cohend_thresh(:,:,2)], 'MissingDataColor', 'w', 'GridVisible', 'off', 'MissingDataLabel', " ",'Colormap',cmap); hp.XDisplayLabels = AxesLabels; caxis([caxis_lower caxis_upper]); hp.YDisplayLabels = nan(size(hp.YDisplayData)); title('Subject - Neighbor')
        ...
        subplot(1,3,3); hp=heatmap(sign(cohend_thresh(:,:,1))-sign(cohend_thresh(:,:,2)), 'MissingDataColor', 'w', 'GridVisible', 'off', 'MissingDataLabel', " ",'Colormap',cmap); hp.XDisplayLabels = AxesLabels; caxis([caxis_lower caxis_upper]); hp.YDisplayLabels = nan(size(hp.YDisplayData)); title('Subject - Neighbor')
...
%     saveas(gcf, [savePath '/Venn_diagram_behavior.png']); pause(3); close all
    ...
    if plot_toggle
        ...
        legend({'Subject','Neighbor'},'Location','best')
