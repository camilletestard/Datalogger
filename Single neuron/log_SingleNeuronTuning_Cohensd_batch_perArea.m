...
for s =session_range %1:length(sessions)
    ...
    n_neurons(1,s) = length(find(strcmp(brain_label,"vlPFC"))); %Get number of neurons
    n_neurons(2,s) = length(find(strcmp(brain_label,"TEO")));

    disp(['session' s])
end

figure; hold on
vlPFC = nan(1,max(session_range)); TEO = nan(1,max(session_range));
for s=session_range
    vlPFC(1,s) = length(find(vertcat(num_selective_behav_per_neuron_vlPFC{s})==0))/n_neurons(1,s);
    TEO(1,s) = length(find(vertcat(num_selective_behav_per_neuron_TEO{s})==0))/n_neurons(2,s);
    
    vlPFC(2,s) = length(find(vertcat(num_selective_behav_per_neuron_vlPFC{s})==1))/n_neurons(1,s);
    TEO(2,s) = length(find(vertcat(num_selective_behav_per_neuron_TEO{s})==1))/n_neurons(2,s);
