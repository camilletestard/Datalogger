...
for s =session_range %1:length(sessions)
    ...
    behavior_log_adj=behavior_log_adj(behavior_log_adj{:,'duration_round'}>time_post*temp_resolution,:);
...
beh=5;
    for beh = 1:length(beh_list)
        ...
        beh_idx_sorted = beh_idx_sorted(behavior_log_adj{beh_idx_sorted,'start_time_round'}+time_post<size(Spike_rasters,1));
        ...
        for n= 1:size(Spike_rasters,2)%135%randi(size(Spike_rasters,2),1,2)
            ...
            for i=1:length(beh_idx_sorted)
                ...
                firing_rate(i,:) = Spike_rasters([onset-time_pre:onset, onset+1:onset+time_post],n);
                behavior(i,:)=behavior_labels([onset-time_pre:onset, onset+1:onset+time_post]);
            end
            ...
            if plot_toggle
                ...
                axis xy
                caxis([0 40])
                xlim([size(firing_rate,2)-20 size(firing_rate,2)-1])
