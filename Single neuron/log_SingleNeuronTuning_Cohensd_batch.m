...
for s=a_sessions
    ...
    prop_not_tuned(s) = length(find(num_selective_behav_per_neuron{s}<1))/n_neurons;
    prop_tuned_more_than_one_behav(s) = length(find(num_selective_behav_per_neuron{s}>1))/n_neurons;
end
legend({sessions(a_sessions).name},'Location','eastoutside')
