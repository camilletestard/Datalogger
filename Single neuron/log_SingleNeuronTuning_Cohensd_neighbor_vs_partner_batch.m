%% Log_SingleNeuronTuning_Cohensd_neighbor_vs_partner
%  This script computes firing rate of individual neuron under different
%  behavioral conditions for both neighbor and partner behavior. 
...
num_either_neighbor_or_partner = nan(max(session_range), nbeh); 
num_neither = nan(max(session_range), nbeh); 
prop_either_neighbor_or_partner = nan(max(session_range), nbeh); 
prop_neither = nan(max(session_range), nbeh); 
num_neighbor_and_partner = nan(max(session_range), nbeh); 
prop_neighbor_and_partner = nan(max(session_range), nbeh); 
num_neighbor_but_notPartner = nan(max(session_range), nbeh); 
prop_neighbor_but_notPartner = nan(max(session_range), nbeh); 
num_partner_but_notPartner = nan(max(session_range), nbeh); 
prop_partner_but_notPartner = nan(max(session_range), nbeh); 
...
for s =session_range %1:length(sessions)
    ...
    savePath = [home '/Dropbox (Penn)/Datalogger/Results/' sessions(s).name '/SingleUnit_results/neighbor_vs_partner'];
   ...
   [Spike_rasters, labels, labels_partner, labels_neighbor, behav_categ, block_times, monkey, reciprocal_set, social_set, ME_final,unit_count, groom_labels_all]= log_GenerateDataToRes_function_neighbor(filePath, temp_resolution, channel_flag, is_mac, with_NC, isolatedOnly);
    ...
    %Extract behavior labels for partner and neighbor
    behavior_labels_subject_init = cell2mat({labels{:,3}}');
    ...
    %Specifcially for Hooke who doesn't self-groom much...
    %behavior_labels_neighbor_init(behavior_labels_neighbor_init==find(behav_categ=="Self-groom"))=find(behav_categ=="Groom partner"); 

    %% Get baseline firing from epochs where the neighbor rests idle.

    %Estimate "baseline" neural firing distribution.
    %idx_rest= setdiff(find(behavior_labels_partner_init ==length(behav_categ)), idx_neighbor);%Get idx of "rest" epochs.
    idx_rest = intersect(find(behavior_labels_subject_init ==length(behav_categ)),...
        intersect(find(behavior_labels_partner_init ==length(behav_categ)),...
        find(behavior_labels_neighbor_init ==length(behav_categ))));
    baseline_firing = Spike_rasters(:,idx_rest);
    mean_baseline = mean(baseline_firing,2);
    std_baseline = std(Spike_rasters(:,idx_rest),0,2);

    %% Get indices for which partner and neighbor do not behav similarly & don't engage in reciprocal behaviors
    % Select behaviors manually
    behav = [4,5,18,24]; %focus on a set of behaviors which happen enough time in both the partner and neighbor and are not reciprocal

    % Get indices where either:
    % 1. The partner is behaving and the neighbor is resting
    idx_sub = find(ismember(behavior_labels_partner_init,behav));
    idx_neighbor = find(ismember(behavior_labels_neighbor_init,behav)); %find the indices of the behaviors considered
    idx_partner = setdiff(idx_sub, idx_neighbor);
    ...
    Spike_count_raster_partner = Spike_count_raster(idx_partner,:);
    behavior_labels_partner = behavior_labels_partner_init(idx_partner);

    %Check what the partner is doing during neighbor behavior.
%     behavior_labels_partner_during_neighbor_behav = behavior_labels_partner_init(idx_neighbor);
%     behavior_labels_neighbor_during_partner_behav = behavior_labels_neighbor_init(idx_partner);
%     figure; hold on; subplot(2,1,1); hist(behavior_labels_neighbor, 30); title('Neighbor behavior'); subplot(2,1,2); hist(behavior_labels_partner_during_neighbor_behav, 30); title('Partner behavior during neighbor idx')
%     figure; hold on; subplot(2,1,1); hist(behavior_labels_partner, 30); title('Partner behavior'); subplot(2,1,2); hist(behavior_labels_neighbor_during_partner_behav, 30); title('Neighbor behavior during partner idx')
    ...
    for n = 1:n_neurons(s)
        ...
        for b = 1:n_behav
            ...
            idxs = find(behavior_labels_partner == unqLabels(b));
            ...
            if n_per_behav(b,1)>10 & n_per_behav(b,2)>10
                ...
                %Comparing neighbor and partner
    ...
    %     subplot(1,3,1); hp=heatmap(cohend(:,:,1), 'MissingDataColor', 'w', 'GridVisible', 'off', 'MissingDataLabel', " ",'Colormap',cmap); hp.XDisplayLabels = AxesLabels; caxis([caxis_lower caxis_upper]); hp.YDisplayLabels = nan(size(hp.YDisplayData)); title('Cohens-d heatmap partner')
    ...
    %     subplot(1,3,3); hp=heatmap(cohend(:,:,3), 'MissingDataColor', 'w', 'GridVisible', 'off', 'MissingDataLabel', " ",'Colormap',cmap); hp.XDisplayLabels = AxesLabels; caxis([caxis_lower caxis_upper]); hp.YDisplayLabels = nan(size(hp.YDisplayData)); title('Cohens-d heatmap partner vs. neighbor')

    if plot_toggle
        figure; hold on; set(gcf,'Position',[150 250 1000 400]);
        subplot(1,4,1); hp=heatmap(cohend_thresh(:,:,1), 'MissingDataColor', 'w', 'GridVisible', 'off', 'MissingDataLabel', " ",'Colormap',cmap); hp.XDisplayLabels = AxesLabels; caxis([caxis_lower caxis_upper]); hp.YDisplayLabels = nan(size(hp.YDisplayData)); title('Partner')
        subplot(1,4,2); hp=heatmap(cohend_thresh(:,:,2), 'MissingDataColor', 'w', 'GridVisible', 'off', 'MissingDataLabel', " ",'Colormap',cmap); hp.XDisplayLabels = AxesLabels; caxis([caxis_lower caxis_upper]); hp.YDisplayLabels = nan(size(hp.YDisplayData)); title('Neighbor')
        subplot(1,4,3); hp=heatmap([cohend_thresh(:,:,1)-cohend_thresh(:,:,2)], 'MissingDataColor', 'w', 'GridVisible', 'off', 'MissingDataLabel', " ",'Colormap',cmap); hp.XDisplayLabels = AxesLabels; caxis([caxis_lower caxis_upper]); hp.YDisplayLabels = nan(size(hp.YDisplayData)); title('Partner - Neighbor')
        subplot(1,4,4); hp=heatmap(cohend_thresh(:,:,3), 'MissingDataColor', 'w', 'GridVisible', 'off', 'MissingDataLabel', " ",'Colormap',cmap); hp.XDisplayLabels = AxesLabels; caxis([caxis_lower caxis_upper]); hp.YDisplayLabels = nan(size(hp.YDisplayData)); title('Partner vs. neighbor')
        sgtitle(['p<' num2str(cutoff)])
        saveas(gcf, [savePath '/Cohend_heatmap_all_units_SUBJECT_VS_PARTNER.png']);  pause(3);

        figure; hold on; set(gcf,'Position',[150 250 1000 400]);
        subplot(1,3,1); hp=heatmap(sign(cohend_thresh(:,:,1)), 'MissingDataColor', 'w', 'GridVisible', 'off', 'MissingDataLabel', " ",'Colormap',cmap); hp.XDisplayLabels = AxesLabels; caxis([caxis_lower caxis_upper]); hp.YDisplayLabels = nan(size(hp.YDisplayData)); title('Partner')
        subplot(1,3,2); hp=heatmap(sign(cohend_thresh(:,:,2)), 'MissingDataColor', 'w', 'GridVisible', 'off', 'MissingDataLabel', " ",'Colormap',cmap); hp.XDisplayLabels = AxesLabels; caxis([caxis_lower caxis_upper]); hp.YDisplayLabels = nan(size(hp.YDisplayData)); title('Neighbor')
        subplot(1,3,3); hp=heatmap(sign(cohend_thresh(:,:,1))-sign(cohend_thresh(:,:,2)), 'MissingDataColor', 'w', 'GridVisible', 'off', 'MissingDataLabel', " ",'Colormap',cmap); hp.XDisplayLabels = AxesLabels; caxis([caxis_lower caxis_upper]); hp.YDisplayLabels = nan(size(hp.YDisplayData)); title('Partner - Neighbor')
    ...
    median_cohend_per_behav_partner(s,:) = nanmedian(cohend_thresh(:,:,1));
    num_selective_per_behav_partner(s,:) = sum(~isnan(cohend_thresh(:,:,1)));
    prop_selective_per_behav_partner(s,:) = sum(~isnan(cohend_thresh(:,:,1)))/n_neurons(s);
    ...
    for c = 1:length(behav)
        num_either_neighbor_or_partner(s,c) = length(unique([row_sub(col_sub==c); row_part(col_part==c)]));
        num_neither(s,c) = n_neurons(s) - num_either_neighbor_or_partner(s,c);
        prop_either_neighbor_or_partner(s,c) = num_either_neighbor_or_partner(s,c)/n_neurons(s);
        prop_neither(s,c) = 1-prop_either_neighbor_or_partner(s,c);

        num_neighbor_and_partner(s,c)= length(intersect(row_sub(col_sub==c), row_part(col_part==c)));
        prop_neighbor_and_partner(s,c) = num_neighbor_and_partner(s,c)/n_neurons(s);

        num_neighbor_but_notPartner(s,c) = length(setdiff(row_part(col_part==c), row_sub(col_sub==c)));
        prop_neighbor_but_notPartner(s,c) = num_neighbor_but_notPartner(s,c)/n_neurons(s);

        num_partner_but_notPartner(s,c) = length(setdiff(row_sub(col_sub==c), row_part(col_part==c)));
        prop_partner_but_notPartner(s,c) = num_partner_but_notPartner(s,c)/n_neurons(s);

%         subplot(1,length(behav),c)
%         A = row_sub(col_sub==c);
%         B = row_part(col_part==c);
%         setListData = {A, B};
%         setLabels = ['Partner';'Neighbor'];
...
%     saveas(gcf, [savePath '/Venn_diagram_behavior.png']); pause(3); close all
    ...
    if plot_toggle
        ...
        [~, idx_sort]=sort(median_cohend_per_behav_partner(s,:));
        ...
        legend({'Partner','Neighbor'},'Location','best')
        ...
        set(gca,'FontSize',15);
        saveas(gcf, [savePath '/Distribution_cohend_neighbor_vs_partner.png']); pause(3); close all
...
[~, idx_sort]=sort(nanmean(median_cohend_per_behav_partner));
scatter(1:length(idx_sort),median_cohend_per_behav_partner(:,idx_sort),60,'filled','MarkerFaceAlpha',.7, 'MarkerFaceColor','b')
...
figure
venn([mean(nanmean(prop_selective_per_behav_neighbor)), mean(nanmean(prop_selective_per_behav_partner))], ...
    mean(nanmean(prop_neighbor_and_partner)));
set(gca,'xtick',[])
set(gca,'ytick',[])
...
% % % % 
% % % %     subplot(1,length(behav),c)
% % % %     %venn([nanmean(prop_selective_per_behav_partner(:,c)), nanmean(prop_selective_per_behav_neighbor(:,c))], nanmean(prop_neighbor_and_partner(:,c))); 
% % % %     venn([nanmean(prop_selective_per_behav_neighbor(:,c)), nanmean(prop_selective_per_behav_partner(:,c))], nanmean(prop_neighbor_and_partner(:,c))); 
% % % %     numbers(:,c)=[nanmean(num_neither(:,c)), nanmean(num_neighbor_and_partner(:,c)), ...
% % % %                   nanmean(num_neighbor_but_notPartner(:,c)), nanmean(num_partner_but_notPartner(:,c))];
% % % %     set(gca,'xtick',[])
% % % %     set(gca,'ytick',[])
...
for c = 1:length(behav)
    ...
    venn([nanmean(prop_selective_per_behav_neighbor(a_sessions,c)), nanmean(prop_selective_per_behav_partner(a_sessions,c))], ...
        nanmean(prop_neighbor_and_partner(a_sessions,c)));
    set(gca,'xtick',[])
    set(gca,'ytick',[])
    %a =gca; set(a,'box','off','color','none')
    title(behav_categ(behav(c)))

    numbers_a(:,c)=[nansum(num_neither(a_sessions,c)), nansum(num_neighbor_and_partner(a_sessions,c)), ...
                  nansum(num_neighbor_but_notPartner(a_sessions,c)), nansum(num_partner_but_notPartner(a_sessions,c))];

    subplot(2,length(behav),c+4)
    venn([nanmean(prop_selective_per_behav_neighbor(h_sessions,c)), nanmean(prop_selective_per_behav_partner(h_sessions,c))], ...
        nanmean(prop_neighbor_and_partner(h_sessions,c)));
    set(gca,'xtick',[])
    set(gca,'ytick',[])
    %a =gca; set(a,'box','off','color','none')
    %title(behav_categ(behav(c)))
    ...
    numbers_h(:,c)=[nansum(num_neither(h_sessions,c)), nansum(num_neighbor_and_partner(h_sessions,c)), ...
                  nansum(num_neighbor_but_notPartner(h_sessions,c)), nansum(num_partner_but_notPartner(h_sessions,c))];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Bar plot number of neurons selective for neighbor, partner, both, neither
...
bp = bar([nanmean(nanmean(prop_neither(a_sessions,:),2)); ...
    ...
    nanmean(nanmean(prop_neighbor_and_partner(a_sessions,:),2))],'FaceColor','flat','FaceAlpha',0.2);
...
sp1 = scatter(ones(1,length(a_sessions))*3,nanmean(prop_selective_per_behav_partner(a_sessions,:),2), 'filled','r');
sp1 = scatter(ones(1,length(a_sessions))*4,nanmean(prop_neighbor_and_partner(a_sessions,:),2), 'filled','b');

ylabel('Proportion of units'); ylim([0 0.7]); 
xticks([0.8 1 2 3 4 4.2]);xticklabels({'','neither','neighbor','partner','both',''})
ax = gca;
ax.FontSize = 16;
title('Monkey A')
...
bp = bar([nanmean(nanmean(prop_neither(h_sessions,:),2)); ...
    ...
    nanmean(nanmean(prop_neighbor_and_partner(h_sessions,:),2))],'FaceColor','flat','FaceAlpha',0.2);
bp.CData(1,:) = [0.5 0.5 0.5]; bp.CData(2,:) = [0 1 1]; bp.CData(3,:) = [1 0 0]; bp.CData(4,:) = [0 0 1];
...
sp1 = scatter(ones(1,length(h_sessions))*3,nanmean(prop_selective_per_behav_partner(h_sessions,:),2), 'filled','r');
sp1 = scatter(ones(1,length(h_sessions))*4,nanmean(prop_neighbor_and_partner(h_sessions,:),2), 'filled','b');

ylim([0 0.7]); xticks([0.8 1 2 3 4 4.2]);xticklabels({'','neither','neighbor','partner','both',''})
ax = gca;
ax.FontSize = 16;
title('Monkey H')
