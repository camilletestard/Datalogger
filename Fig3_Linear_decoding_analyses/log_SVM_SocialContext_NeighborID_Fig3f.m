%% Log_SVM_SocialContext_NeighborID_Fig3f
% Run a linear decoder on a the neural activity to decode the identity of
% neighboring monkey during a given behavior.
...
for s =session_range %1:length(sessions)
    ...
    for channel_flag = ["vlPFC", "TEO", "all"]
        ...
        block_labels = cell2mat({labels{:,12}}'); %Extract block label
        paired_labels = cell2mat({labels{:,13}}'); %Extract whether paired or alone info

        Spike_count_raster = Spike_count_raster(paired_labels==1,:);
        behavior_labels = behavior_labels(paired_labels==1);
        block_labels = block_labels(paired_labels==1);
