%% log_preprocess_video_allTracking
%Script which preprocesses and combines movement data from multiple video angles.
%C. Testard Jan 2023
...
session_range = [3:6,11:13,15:16,18]; %sessions with valid video data
prop_missing_data_body = nan(1,max(session_range));
prop_missing_data_logger = nan(1,max(session_range));
prop_missing_by_bp = nan(max(session_range),17);
...
for s =session_range %1:length(sessions)
    ...
    %Forehead x,y,likelihood (2:4)
    %TopRightlogger (5:7)
    %BottomLeftlogger (8:10)
    ...
    tdl_logger_top = table2array(tdl_video(:,5:7)); tdl_logger_top(tdl_logger_top(:,3)<c_cutoff_logger,1:2)=nan;
    tdl_logger_bottom = table2array(tdl_video(:,8:10)); tdl_logger_bottom(tdl_logger_bottom(:,3)<c_cutoff_logger,1:2)=nan;

    tdr_logger_top = table2array(tdr_video(:,5:7)); tdr_logger_top(tdr_logger_top(:,3)<c_cutoff_logger,1:2)=nan;
    tdr_logger_bottom = table2array(tdr_video(:,8:10)); tdr_logger_bottom(tdr_logger_bottom(:,3)<c_cutoff_logger,1:2)=nan;
    ...
    idx_to_replace = tdl_logger_top(:,3)<c_cutoff_logger; %Find indices where certainty about location is low in left camera
    ...
    prop_missing_data_logger(s) = length(invalidpoints_logger)/size(toplogger,1);
...
%     legend({'Left camera', 'Right camera','Combined'})
    ...
    %figure; plot(acceleration)
        ...
        prop_missing_by_bp(s,:)=sum(bodypose(:,c_columns)<c_cutoff_body)./size(bodypose,1);
        ...
        invalidpoints = find(bodypose(:,4)<c_cutoff_body | bodypose(:,7)<c_cutoff_body | bodypose(:,10)<c_cutoff_body...
            | bodypose(:,13)<c_cutoff_body | bodypose(:,16)<c_cutoff_body | bodypose(:,19)<c_cutoff_body | bodypose(:,22)<c_cutoff_body...
            | bodypose(:,25)<c_cutoff_body | bodypose(:,28)<c_cutoff_body | bodypose(:,31)<c_cutoff_body | bodypose(:,34)<c_cutoff_body ...
            | bodypose(:,37)<c_cutoff_body | bodypose(:,40)<c_cutoff_body | bodypose(:,43)<c_cutoff_body | bodypose(:,46)<c_cutoff_body ...
            | bodypose(:,49)<c_cutoff_body | bodypose(:,52)<c_cutoff_body); length(invalidpoints)/size(bodypose,1)
        bodypose(invalidpoints,:)=nan;

        prop_missing_data_body(s) = length(invalidpoints)/size(bodypose,1);%proportion missing data
    ...
    writetable(mvmt_data,['mvmt_data_c' num2str(c_cutoff_body) '.csv'])

    disp(['Session ' sessions(s).name ' done.'])

    clearvars -except home sessions s session_range prop_missing_data_logger prop_missing_data_body c_cutoff_body c_cutoff_logger prop_missing_by_bp


end


bodyparts_lbls={'Nose','left eye','right eye','left ear','right ear',...
...
figure; hold on
plot(prop_missing_by_bp');
plot(nanmean(prop_missing_by_bp),'LineWidth',3)
yline(0.85,'LineStyle','--')
ylabel('% Missing data'); ylim([0 1])
title(['Certainty score: ' num2str(c_cutoff_body)])
xticks([1:17])
xticklabels(bodyparts_lbls)

%Data missing body tracking
nanmean(prop_missing_data_body)
nanstd(prop_missing_data_body)
%c = 0.6: 98.72% (SD 0.6%)
%c = 0.2: 71.46% (SD 11.27%)

%Data missing logger tracking
nanmean(prop_missing_data_logger)
nanstd(prop_missing_data_logger)
%c = 0.7: 53.86% (SD 9.5%)
