...
for s =session_range(6:end) %1:length(sessions)
    ...
    chan = 1;
    %for channel_flag = ["all", "vlPFC", "TEO"]

        channel_flag = "all"; %2022-12-13: Probably not, but let's check if its a brain region thing/number of neurons thing
        ...
        save(['~/Dropbox (Penn)/Datalogger/Results/All_sessions/Mvmt_results/ridgeRegResults_alone.mat'], 'RegGroups','Rsq_unique','Rsq_full', 'RsqFull')
