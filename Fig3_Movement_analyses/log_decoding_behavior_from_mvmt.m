%% Log_decoding_behavior_from_mvmt.m
% Run umap and a linear decoder on movement data alone for the subject's behavior
% Compare to decoder on movement data. 
...
for s =session_range
    ...
    idx_beh = find(ismember(behavior_labels,behav)); %find the indices of the behaviors considered

    behavior_labels_final = behavior_labels(idx_beh,:);%Same as above but in behavior labels
    ...
    for pred = 1:2
        ...
        if pred == 1
            predictors_final = Y_final(idx_beh,:);
        else
            predictors_final = mvmt_final(idx_beh,:);%Only keep timepoints where the behaviors of interest occur in spiking data
        end
        ...
        for iter = 1:num_iter
            ...
            if randomsample==1
                Input_matrix = predictors_final(:,randsample(unit_count(chan), min(unit_count)));
            else
                Input_matrix = predictors_final;
            end
        ...
        mean_hitrate{s}(pred) = mean(hitrate)
        sd_hitrate{s}(pred) = std(hitrate);
        mean_hitrate_shuffled{s}(pred) = mean(hitrate_shuffled)
        sd_hitrate_shuffled{s}(pred) = std(hitrate_shuffled);

        C_concat=cat(3,C{:}); %Get confusion matrix
        confusion_mat_avg{s, pred}=round(mean(C_concat,3)*100); %Average over SVM iterations
...
save(['SVM_from_mvmts_c' num2str(c_cutoff) '.mat'], "mean_hitrate","sd_hitrate","mean_hitrate_shuffled","behav","a_sessions","h_sessions","behav_categ")
...
data_relative(:,2) = data_shuffle(:,1)./data(:,1);
...
sp1 = scatter(ones(size(data,1))*3,data_shuffle(:,1), 'filled','g');
ylabel('Decoding Accuracy'); ylim([0 1])
xticks([1:4]); xticklabels({'Neural activity', 'Mvmt', 'Chance'}); xlim([0.25 4.75])
ax = gca;
ax.FontSize = 16;
saveas(gcf, ['SVM_Decoding_from_mvmt_c' num2str(c_cutoff) '.pdf'])
