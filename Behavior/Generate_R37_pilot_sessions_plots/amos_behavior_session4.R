...
#Load data:
setwd('C:/Users/Camille Testard/Desktop/Grants_Fellowships/R37_Project/pilot_data/session4')
behavioral_log = read.csv('Behavioral_log.csv')
...
awakeness = behavs[c(2,4,10,16)] #sleep/wake states
behavs = behavs[-c(1,2,3,4,10,11,16,18)] #behaviors
stress_time = log$Time[log$Behavior=="Stressed"];
fully_awake_time = log$Time[log$Behavior=="Fully awake"];
...
for (b in 1:length(behavs)){
  ...
  data=log[log$Behavior==behavs[b],c("Time", "Behavior", "Start.Stop","stressed","simultaneous_behav")]
  ...
  data = cbind(interim[[1]],interim[[2]]); data=data[,-c(3,4,5,7,8)]
  data=data[,c("Behavior","start.time","stop.time","stressed","simultaneous_behav")]
...
times = c(log$Time[which(log$Behavior=="Waking up")],
          log$Time[which(log$Behavior=="Fully awake")])
new_log[nrow(new_log)+1,"Behavior"]="Asleep"
new_log[nrow(new_log),"start.time"]=1; new_log[nrow(new_log),"stop.time"]=times[1];
new_log[nrow(new_log),"stressed"]="no"; new_log[nrow(new_log),"simultaneous_behav"]="no";
...
resting$Behavior="Resting"; resting$stressed="no"; resting$stressed[resting$start.time>stress_time]="yes";
resting$simultaneous_behav="no";
...
#Save to .csv
write.csv(new_log[,-c(7,8)],file='behav_log_restructured.csv',row.names = F)

#Plot
behavior.log<-ggplot(new_log, aes(xmin=start.time, xmax= stop.time, ymin=group.min, ymax=group.max))+
  geom_rect(aes(fill=Behavior))+#, colour = "grey50")+
  theme_classic(base_size = 20)+ ylim(0,1)+xlim(0,max(new_log$stop.time))+
  geom_vline(xintercept=wakingup_time, lwd=2)+ geom_vline(xintercept=fully_awake_time,lwd=2)+ 
  geom_vline(xintercept=stress_time,lwd=2)+
  xlab('Time since start of recording (in s)')+
  theme(axis.text.y= element_blank(),
        axis.ticks.y = element_blank())+
  scale_x_continuous(breaks=c(0,600,2000,4000,6000, 8000))
...
# new_log2[1,c("Behavior","start.time","stop.time")]=c("sleep",1,times[2])
# new_log2[2,c("Behavior","start.time","stop.time")]=c("waking.up",times[2],times[3])
# new_log2[3,c("Behavior","start.time","stop.time")]=c("awake",times[3],max(new_log$stop.time))
# new_log2$start.time = as.numeric(new_log2$start.time); new_log2$stop.time = as.numeric(new_log2$stop.time)
# new_log2$duration.s = new_log2$stop.time-new_log2$start.time
# new_log2$group.min=1; new_log2$group.max=2

# #Combine the two
# final_log=rbind(new_log,new_log2)

# ggplot(final_log, aes(xmin=start.time, xmax= stop.time, ymin=group.min, ymax=group.max))+
...
#other package:
# timeline(new_log, label.col='Behavior', start.col='start.time', end.col='stop.time',
#          group.col='Group')
