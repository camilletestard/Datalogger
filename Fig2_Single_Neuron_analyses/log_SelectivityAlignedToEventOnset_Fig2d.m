%% Log_SelectivityAlignedToEventOnset_Fig2D.m
% Measure the behavior transition index (neural response index) for each unit
% aligned to event ONSET (Fig 2D).
% C. Testard, July 2023
        ...
        if num_events(s,beh)>0
            ...
            for n= 1:size(Spike_rasters,2)%135%randi(size(Spike_rasters,2),1,2)
                ...
                for i=1:length(beh_idx)
                    onset = behavior_log_adj{beh_idx(i),'start_time_round'};
