%% Log_SelectivityAlignedToEventOffset.m
% Produce peri-event time hitograms, aligned to event offsets.
% C. Testard, July 2023
...
for s =session_range %1:length(sessions)
    ...
    beh_list = {'Aggression','Foraging','Groom Give','Groom Receive','Self-groom','Travel','Drinking'};
    ...
    for beh = 1:length(beh_list)
        ...
        beh_idx=beh_idx(behavior_log_adj{beh_idx,'end_time_round'}+time_post<size(Spike_rasters,1));
        ...
        if num_events(s,beh)>0
            ...
            for n= 1:size(Spike_rasters,2)%135%randi(size(Spike_rasters,2),1,2)
                ...
                for i=1:length(beh_idx)
                    offset = behavior_log_adj{beh_idx(i),'end_time_round'};
