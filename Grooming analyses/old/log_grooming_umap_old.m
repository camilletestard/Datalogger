...
%% Plot trajectory of grooming bouts

for b=1:length(bouts_to_consider)
    idx = groombouts(bouts_to_consider(b),10):groombouts(bouts_to_consider(b),11);
    timescale = rescale(idx);
    bout_id = ones(size(idx))*b;
    ...
    %plot3()
    figure; hold on
    plot3(smoothdata(umap_result{s}(idx,1)), smoothdata(umap_result{s}(idx,2)),smoothdata(umap_result{s}(idx,3)))
    scatter3(smoothdata(umap_result{s}(idx,1)), smoothdata(umap_result{s}(idx,2)),smoothdata(umap_result{s}(idx,3)),40,timescale,'filled')%Cmap_recip(recip,:),'filled')
    xlabel('UMAP 1'); ylabel('UMAP 2'); zlabel('UMAP 3')
    colormap(hot)
    colorbar
    %set(gca,'xtick',[]); set(gca,'ytick',[]); set(gca,'ztick',[])
    ...
    figure; hold on
    plot3(umap_result{s}(idx,1), umap_result{s}(idx,2),umap_result{s}(idx,3))
    scatter3(umap_result{s}(idx,1), umap_result{s}(idx,2),umap_result{s}(idx,3),40,recip_id,'filled')%Cmap_recip(recip,:),'filled')
    xlabel('UMAP 1'); ylabel('UMAP 2'); zlabel('UMAP 3')
    colormap(cool); caxis([1 2])
    colorbar
    %set(gca,'xtick',[]); set(gca,'ytick',[]); set(gca,'ztick',[])
    set(gca,'FontSize',12);
    title('Time in bout')
...
figure
scatter3(umap_result{s}(idx_final,1), umap_result{s}(idx_final,2),umap_result{s}(idx_final,3),20,threatid_final,'filled')%Cmap_recip(recip,:),'filled')
xlabel('UMAP 1'); ylabel('UMAP 2'); zlabel('UMAP 3')
colormap(parula)
colorbar
%set(gca,'xtick',[]); set(gca,'ytick',[]); set(gca,'ztick',[])
...
figure
scatter3(umap_result{s}(idx_final,1), umap_result{s}(idx_final,2),umap_result{s}(idx_final,3),20,recipid_final,'filled')%Cmap_recip(recip,:),'filled')
xlabel('UMAP 1'); ylabel('UMAP 2'); zlabel('UMAP 3')
colormap(summer)
colorbar
%set(gca,'xtick',[]); set(gca,'ytick',[]); set(gca,'ztick',[])
...
s=1;
groombouts = allGroomBouts_sorted_save{s};
groombouts = groombouts(groombouts(:,3)>10,:);
neural_data = zscore(Spike_count_raster{s});
padding = 500;
...
[isort1_all] = mapTmap(average_firing_rate);
figure; imagesc(average_firing_rate'); colorbar
