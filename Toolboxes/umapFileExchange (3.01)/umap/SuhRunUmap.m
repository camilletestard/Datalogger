...
classdef SuhRunUmap < handle
    ...
    methods 
        function this=SuhRunUmap(nArgOut, varargin)
            ...
            curAxes=[];
            if beGraphic
                %init top level  varaibles set by prepareGui
                firstPlot=true;xLabel=[];yLabel=[];zLabel=[];
                askedToSeeOutputFolder=false;
            
            
                [fig, curAxes, tb, btnPolygon, cbSyncKld, comboClu, ...
                    lblClu, btnNameRoi]=prepareGui;
                if ~handleNoData
                    return;
                end
            end
            
            %init top level  varaibles set by prepareData
            ...
            function reduceHiD2LoD
                ...
                if args.match_predictions 
                    if strcmp(UMAP.REDUCTION_TEMPLATE, args.reductionType)...
                        ...
                        warning(['Reduction is %s ... match_predictions '...
                            'for basic or supervised template '...
                            'reductions'], args.reductionType);
                    elseif isempty(testSetLabels)
                        warning(['match_supervised requires label_column'...
                ...
                if ischar(csv_file_or_data)
                    ...
                    if beGraphic
                        ...
                        if dd.bytes>10*1000000 % 10 million
                            ...
                            an=Gui.ImageLabel(loadingHtml, 'genieSearch.png', ...
                                '',[],fig);
                            pos=get(an,'position');
            ...
            function progressPopUp=initProgressPopUp
                ...
                try
                    ...
                    if args.fast_approximation
                        ...
                        ttl='Reducing parameters...';
                    end
                    imgFile=Gui.GetResizedImageFile('Genie.png', .5);
                    progressPopUp=PopUp(Html.WrapHr(sprintf(['Using UMAP to reduce '...
            ...
            function setClusterCount
                ...
                if isempty(dbm) || ~strcmpi('dbm', dbm.method)
                    html='DBM cluster detail ';
                else
                    html=sprintf('%d DBM clusters, detail is ', dbm.numClusters);
                end
                lblClu.setText(Html.WrapSmallBold( html, globals));
            end
            ...
            function [fig, ax,tb,btnPolygon,cbSyncKld, comboClu, ...
                ...
                if isempty(args.locate_fig)
                    if args.see_training
                        fig=Gui.Figure(false, 'run_umap_fig2',[],whereIf1stTime);
                    else
                        fig=Gui.Figure(false, 'run_umap_fig',[],whereIf1stTime);
                    end
                else
                    fig=Gui.Figure(false);
                end
                ...
                if args.roi_table==1 || args.roi_table==3
                    ...
                    cbSyncKld=Gui.CheckBox(...
                        Html.WrapSmallBold(['Sync ' img], globals), ...
                ...
                if args.n_components==2
                    ...
                    lblClu=Gui.Label(Html.WrapSmallBold('100 DBM clusters, detail is ', globals));
                    jp.add(lblClu);
                    comboClu=Gui.Combo(items, 0, '',[],@(h,e)clusterCallback(h), ...
                        'Select a type of clustering');
                    jp.add(comboClu);
            ...
            function clusterCallback(h)
                ...
                if h.getSelectedIndex<1
                    ...
                    if isempty(dbm) || ~isequal(dbm.detail, detail)
                        [~,~,dbm]=Density.FindClusters(reduction, detail);
                    end
