...
%                           Default is 0.3.
%
%   'spread'                Controls the effective distance scale of
%   (v3.0)                  embedded points as does the same input argument
%                           for the original implementation. Modifying this
%                           value requires the Curve Fitting Toolbox.
%                           Default is 1.
%
...
%                           Default is 15. 
%   
%   'metric'                A synonym argument name is 'Distance'.  This 
%                           Controls how distance is computed in the
...
%                           1 matches supervised and supervising data
...
%                           4 is similar to 2 except it only uses closeness
...
%   (v3.0)                  window.  This contains a table of mass+distance
...
%                           adjudicate classification disagreement 
%                           by comparing the prior classification's
%                           false negatives to the UMAP classification's
%                           false positives ... and then comparing both 
%                           to the agreed upon true positives as well as 
%                           the prior classification's predicted subset.
%                           The prerequisites for this are: a) a prior
%                           classification is conveyed by the ‘label_column’
%                           argument; b)UMAP’s HiD to LoD reduction is 
%                           either basic or by a supervised template ... NOT 
%                           by a supervised or template reduction.
%                           Default is false.
%
%
...
% 
%   'locate_fig'           When verbose is graphic this cell data
%   (v3.0)                 indicates how to position the umap
%                          figure relative a parent figure.
%                          The cell as 3 data items: 1=parent figure, 
%                          2=location (west/east/sout/north0,
%                          3=true if close when parent fig
%                          Default is {} (no parent locating). 
%
%   'save_output'          Capture figures in png files
%   (v3.0)                 Default is false
% 
%   'output_folder'        Folder to deposit png files into
%   (v3.0)                 Default is same folder as csv file or
%                          ~/Documens/run_umap if no csv file
% 
...
%   (v3.0)                 Compression uses our lab's probability binning                             
%                          algorithm described at:
%                          https://onlinelibrary.wiley.com/doi/full/10.1002/1097-0320(20010901)45:1%3C37::AID-CYTO1142%3E3.0.CO%3B2-E
%                          Probability bins represent open covers of a more
%                          basic kind than UMAP’s simplicial complexes.  
%                          For most data sets the loss of accuracy with
%                          this approximation is negligible.  In example 27
%                          UMAP's reduction of 1.8+million X 27 measurements 
%                          takes ~17 minutes on our 6 core MacBook ... but 
%                          with fast approximation it takes ~20 seconds yet
%                          this speed up costs little accuracy when 
%                          comparing UMAP's data islands to cell subsets 
%                          defined by by expert biologists:  fast
%                          approximation forms data islands that are ~97% 
%                          similar ... compared to ~99% similarity without
%                          fast approximation.  The merits of approximation
%                          for particular data sets can be tested wtih 
%                          automatic computation of ROI polygons for 
%                          populations with classification labels as
%                          described below in roi_percent_closest.
%                          Default is false.
%           
%   'roi_percent_closest'  % of a labeled subset to include within the
%                          boundaries of a computed polygon.
%                          This is used when verbose=graphic, n_components 
%                          is 2 and label_column is not empty.  Users pick
%                          labeled population(s) and then click the button
%                          "Regions of interest" south of the legend window
%                          to compute an automatic polygon ROI around the 
%                          regions where MOST of their labeled data points 
%                          lay. MOST is based on euclidean closeness to
%                          the median of the data with the same labels.
%                          Default is .94 if fast_approximation is false
%                          and .91 if fast_approximation is true.
%                          
...
%       cells.
%
%       run_umap sampleRag148k.csv template_file ustBalbc2D.mat cluster_detail medium match_supervisors 1;
%
...
%       plot pair.
%
%       run_umap sampleBalbcLabeled55k.csv label_coxlumn 11 label_file balbcLabels.properties qf_tree true n_components 3 save_template_file ustBalbc3D.mat;
...
%       Also set match_predictions true to inspect similarity of 
%       prior classifications to true+, false+ and false- results.
%
...
%
%       WITH NN descent acceleration and fast_approimation:
%   
...
%
%   23. Observe the "prediction strength/quality" of the data islands 
%       formed by basic unsupervsed reductions when matched to prior 
%       classifications labels made manually by expert biologists to define
%       lymphocyte cell subpopulations. 
%
...
%       
%       Set argument 1 to the following files to see UMAP's power
%       when compared to the manual classifications of separate
%       published data sets.
%
...
%       
%       
%       After any/all of the above runs complete this command extracts the
%       true+, false+ and false- summary shown on the 
%       PredicutionAdjudicator window at the top left.
...
%       
%       This command extracts  the match summary shown at the top left of
%       the Mass+distance similarity window.
...
%       
%       These post reduction commands provide 
%       And these commands pull out details of subset match records for top 2
%       matches, worst 2 matches and no matches.  See the function
%       getMatches in UMAP_extra_results.m for how to programatically 
...
%       data predefined by expert biologists who used 35 biomarkers and 
%       published their result as a standard protocol in
...
%     
%   26. This examples takes example 4's data to illustrate the 'synthesize' 
%       argument to create a synthetic data set. In this case we want to
%       get 24000 rows out of the 55,000 that appear similar and make sure
%       that no single class (defined by label) falls below 30 rows.
%
%       run_umap('sampleBalbcLabeled55k.csv', 'label_column', 11, 'label_file', 'balbcLabels.properties', 'synthesize', [24000 30]);
%
%   27. This example is like example 24 but with a larger data set and
%       higher similarity between the "data islands" formed by basic UMAP
%       reduction and the subsets predefined by expert biologists. This is
%       a standard flow cytometry protocol published in 2018 at
%       https://onlinelibrary.wiley.com/doi/10.1002/cyto.a.23331. Basic
%       UMAP finds ALL of the publication's cell subsets when comparing
%       with BOTH F-measure overlap or mass + distance similarity.  Using
%       fast_approximation the loss of accuracy for both is 3%. UMAP's
%       reduction for 1.8+ million rows and 27 columns takes ~20 seconds on
%       our 6-core MacBook with fast approximation and 17 minutes without.
%       Comparing takes ~2 minutes.
%
...
%
%   28.  This example illustrates using EPP (our lab's unsupervised HiD
%        data classifier) to create data subset labels for supervising
%        UMAP's formation of "data islands".  EPP takes a more 
%        conservative approach to the curse of dimensionlity than does UMAP's 
%        topology and set theory.  The curse of dimensionality is described
%        more fully at:
...
%        classification from basic reductiopn to data islands with clusters.
%
...
%
%   29.  Using the published data set described in example 19 test
%        compare the classification accuracy done by UST reduction to 2D 
%        for all match scenarios versus basic UMAP reduced.
%        
...
%
%
%        These post reduction commands display the average match goodness
%        of ust and basic umap for each of this example's match_scenarios
%        1, 2 and 4.  To understand a simple programmatic access to these
%        results see the showAllMatchScenarios function in the file
%        UMAP_extra_results.m.
%
...
%
%   30.  Reduce parameters for sample30k.csv while adjusting the 'min_dist'
%        and 'spread' parameters to change the output plot. Reducing
%        'min_dist' moves embedded points closer to nearest neighbours,
%        while increasing 'spread' moves other pairs of points farther
...
%
%   REQUIRED PATHS
%   This distribution has 3 folders:  umap, util and epp.  
