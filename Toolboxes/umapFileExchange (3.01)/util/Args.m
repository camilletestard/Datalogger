...
classdef Args < handle
    ...
    methods
        ...
        function setMetaInfo(this, arg, varargin)
            ...
            assert(isfield(this.fields, arg));
        ...
        function pnl=getArgumentClinicPanel(this, description)
            ...
            [pnlList, jList]=Gui.NewListSearch(choices,'Find 1+ args', [], ...
            ...
            this.jtfCommand=Gui.NewTextField('', 35, 'Copy & paste to command line');
            pnlSouth=Gui.FlowLeftPanel(12);
            pnlSouth.add(Gui.NewBtn(...
                Html.WrapSmallBold('Copy command...'), @(h,e)copyCmd(), ...
                'Copy command to clipboard', 'Copy.png'));
            pnlSouth.add(this.jtfCommand);
            pnl=Gui.BorderPanel([],2,15, 'Center', pnlCenter, 'South', pnlSouth);
            ...
            function setFileFociIfNeeded
                if ~isempty(this.fileFoci)
                    ...
                    for i=1:nFoci
                        ...
                        pnlSelectCsv=Gui.Panel(Gui.NewBtn(Html.WrapSmall(...
                            'Select<br>csv file'), @(h,e)changeFileFocus(focus), ...
        ...
        function setFileFocus(this, ttl, args)
            f.args=args;
            f.choices=args;
            if 2==length(args) 
                ...
                if ~isempty(cur)
                    ...
                    if idx>0
                        if ~isempty(this.csv_label_column_args{idx})
                            ...
                            if label_column>0
                                this.update(this.csv_label_column_args{idx},...
                                    label_column);
    ...
    methods
        ...
        function gotLoaded=load(this)
            ...
            haveSavedMetaInfo=isempty(saveFileInfo);
            if ~isdeployed %check uncompiled source code file
                for i=1:length(this.commentFiles)
                    ...
                    if haveSavedMetaInfo ...%saved is OLDER
        ...
        function unknowns=warnUnknowns(this)
            ...
            if unknowns>0
                ...
                warning('%d arguments of unknown tyoe', unknowns);
            end
    ...
    methods(Static)
        function [args, argued, unmatchedArgs, this]=NewKeepUnmatched(p, varargin)
        ...
        function varArgOut=Str2NumOrLogical(args, varArgIn)
            N=length(varArgIn);
            varArgOut=cell(1, N);
            for i=1:2:N
                name=varArgIn{i};
                ...
                varArgOut{i}=name;
                varArgOut{i+1}=arg;
                if ischar(arg) && isfield(args, name)
                   fieldValue=getfield(args, name);
                   if islogical(fieldValue)
                       varArgOut{i+1}=strcmpi(arg, 'true') || strcmpi(arg, 'yes');
                   elseif isnumeric(fieldValue) && ~isempty(fieldValue)
                       strNum=str2double(arg);
                       if ~isnan(strNum)
                           varArgOut{i+1}=strNum;
                       else
                           varArgOut{i+1}=arg;
        ...
        function ok=IsLabelColumn(x)
            ok= strcmpi(x, 'end') || (isnumeric(x) && x>0);
        end
        function ok=IsFileOk(x)
            if ~ischar(x)
                ok=isempty(x);%ok to clear file
            elseif exist(x, 'dir')
                ok=false;
        ...
        function varArgIn=RemoveArg(varArgIn, arg)
            N=length(varArgIn);
            arg=lower(arg);
            for i=1:2:N
                a=lower(varArgIn{i});
