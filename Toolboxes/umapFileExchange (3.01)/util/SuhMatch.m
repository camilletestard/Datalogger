    ...
    methods(Static)
        function [match, matchTable, trainingQfTreeMatch, ...
            ...
            match=[]; matchTable=[]; trainingQfTreeMatch=[]; ...
                trainingQfTree=[]; testQfTreeMatch=[]; testQfTree=[];
            try
                pu=PopUp(Html.WrapHr(['<b>Activating SUH subset matching '...
                    'pipeline</b><br><br>' Html.WrapBoldSmall( ...
                    '(<i>Dr. Orlova''s QFMatch/QF-tree tools</i>)')]), ...
                    'center', 'Subset characterizing...',false,true,...
                    Gui.GetResizedImageFile('orlova.png', .25, app));
                ...
                [trainingNames, trainingClrs, trainingIds]=trainingSet.getLabelInfo;
                ...
                savedFile=fullfile(folder, SuhDataSet.FILE_MATCH);
                ...
                if isempty(testSet.data) || isempty(testSet.labels)
                    ...
                    if isempty(qf) || ~args.ask_if_preexists ...
                            || ~askYesOrNo('Re-use prior Match calculations?')
                        justDone=true;
                    ...
                    if args.match_table_fig || args.match_histogram_fig
                        matchTable=QfTable(match, trainingClrs, [],...
                            get(0, 'currentFig'), visible, [], 'SuhMatch');
                        if justDone
        ...
        function args=CheckOutputFolder(args, trainingSet, testSet)
            if isempty(args.output_folder)
                if ~isempty(trainingSet.data)
                    [p,f1]=fileparts(trainingSet.file);
                    f1=[f1 '_matches'];
                    if ~isempty(testSet.data)
                        [~,f2]=fileparts(testSet.file);
        ...
        function p=DefineArgs()
            p = inputParser;     
            addParameter(p,'training_set', 'panoramaSample6_labeled.csv', @Args.IsFileOk);
            addParameter(p,'training_label_file', '', @Args.IsFileOk);
            addParameter(p,'training_label_column', 'end', @Args.IsLabelColumn);
            
            addParameter(p,'test_set', 'panoramaSample10_labeled.csv', @Args.IsFileOk);
