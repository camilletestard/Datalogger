    ...
    methods
        function this=SuhDataSet(csvFileOrData, args)  
            ...
            if ~isempty(this.labels)
                ...
                args.n_rows=this.R;
                [lblMap_,halt]=LabelBasics.GetOrBuildLblMap(this.labels, args);
                if halt
                    this.labels=[];
                else
                    this.lblMap=lblMap_;
        ...
        function ok=normalize(this)
            ...
            badCols=find(mx>1);
            if ~isempty(badCols)
                msgBox(Html.WrapHr(sprintf([...
                    '%d column(s) not normalized from 0 to 1'...
                    '!<br><br>Will adjust them now however the'...
                    '<br>polygons will not export back to original '...
                    ';data..'],  length(badCols))));
                this.data(:,badCols)=(this.data(:,badCols)-0)/mx(badCols);
            end
        end
                ...
                for i=0:cols-1
                    ...
                    if col>this.C
                        ...
                        if ~isempty(xyIdx)
                            ...
                            mx=this.columnRanges(idx+2);
                            range=mx-mn;
                            xyData(:, xyIdx)=(xyData(:, xyIdx)*range)+mn;
                        end
                    end
                end
            end    
        ...
        function [qf, matchTable]=characterize(this, epp, pu, visible)
            ...
            if ~args.match_table_fig && ...
                ~args.match_histogram_fig  && ~args.qf_tree
                qf=[];
                matchTable=[];
                return;
            end
