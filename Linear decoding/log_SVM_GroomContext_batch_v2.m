...
num_iter = 25;%Number of SVM iterations
...
for s =session_range %1:length(sessions)
    ...
    for channel_flag = ["vlPFC", "TEO", "all"]
        ...
        groom_categ_label={'Post threat','Reciprocal','Bout id','Time in bout','Reciprocity','Cumulative grooming (within session)','Cumulative grooming (over all sessions)'};
        ...
        cumul_groomBout_perSession = cumsum(diff(groom_behav_perSession)); figure; plot(cumul_groomBout_perSession)
        ...
        for behav = [8,7] %For both groom give and groom receive
            ...
            for groom_categ = 2:7 %For all grooming contexts
                ...
                groombouts = allGroomBouts_sorted(ismember(allGroomBouts_sorted(:,6),behav),:);
                ...
                %cumul_groom_over_all_sessions_categ = round(rescale(cumul_groom_over_all_sessions(idx_all))*10);
                ...
                groom_type_label = [threatid_all; recipid_all; boutid_all; timescale_all; recip_groom_categ'; cumul_groom_categ'];
                %[r p] = corrcoef(groom_type_label(:,3:end)'); rho{s}(beh,:)=r(1,2:end);
...
%                 behavior_labels_final = behavior_labels_final(500:1500);
%                 Spike_count_raster_final = Spike_count_raster_final(500:1500,:);
%                 
                disp('########################')
...
%save('SVM_groomingCateg.mat', "groom_categ_label","mean_hitrate","mean_hitrate_shuffled","behav","a_sessions","h_sessions","session_range","behav_categ","home")
load('SVM_groomingCateg.mat')
...
for groomcat = 3:6
    ...
    data = squeeze(mean_hitrate(beh,groomcat,:,session_range))';
    data_shuffled = squeeze(mean_hitrate_shuffled(beh,groomcat,:,session_range))';
