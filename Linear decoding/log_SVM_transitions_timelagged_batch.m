...
%             % Make proximity "rest" and remove all rest epochs
...
%             behavior_labels_subject = behavior_labels_subject_init(behavior_labels_subject_init~=length(behav_categ));
            
            %
            behavior_labels_subject = behavior_labels_subject_init;
...
xticks([0.8 1 2 3 3.2]); xlim([0.8 3.2]); ylim([0 5])
...
if transition ==1
    saveas(gcf,['SVM_results_ANY_state_transitions_allSessions.png'])
else
    saveas(gcf,['SVM_results_state_SPECIFIC_transitions_allSessions.png'])
end