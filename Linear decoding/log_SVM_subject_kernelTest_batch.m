    ...
    for kernel = 0:3
        ...
        for channel_flag = ["vlPFC", "TEO", "all"]
            ...
            for iter = 1:num_iter
                ...
                % Run svm
                [hitrate(iter), C{iter}] = log_SVM_basic_function_kernelTest(Input_matrix, Labels, 5, 0, 0, kernel);
                [hitrate_shuffled(iter), C_shuffled{iter}] = log_SVM_basic_function_kernelTest(Input_matrix, Labels_shuffled, 5, 0, 0, kernel);
            ...
            mean_hitrate{s}(kernel+1, chan) = mean(hitrate)
            sd_hitrate{s}(kernel+1, chan) = std(hitrate);
            mean_hitrate_shuffled{s}(kernel+1, chan) = mean(hitrate_shuffled)
            ...
            C_table{s, kernel+1, chan} = array2table(confusion_mat_avg,'RowNames',rowNames{s},'VariableNames',colNames{s});

            chan = chan +1;
            clear labels_id

        end %end of channel loop
    end %end of kernel for loop
