    ...
    for channel_flag = ["vlPFC", "TEO", "all"]
        ...
        for behav = [8,7] %For both groom give and groom receive
            ...
            for iter = 1:num_iter
                ...
                Labels = behavior_labels_final; figure; plot(Labels)
                ...
                Labels_shuffled = Labels(randperm(length(Labels)));figure; plot(Labels_shuffled)

                % Run svm
                [hitrate(iter), C{iter}] = log_SVM_basic_function(Input_matrix, Labels', 2, 0, 0);
                [hitrate_shuffled(iter), C_shuffled{iter}] = log_SVM_basic_function(Input_matrix, Labels_shuffled', 2, 0, 0);
            ...
            mean_hitrate(beh,chan,s) = mean(hitrate);
            mean_hitrate_shuffled(beh,chan,s) = mean(hitrate_shuffled);
