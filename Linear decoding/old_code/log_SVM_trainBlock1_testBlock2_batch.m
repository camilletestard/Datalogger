...
for s =session_range %1:length(sessions)
    ...
    for channel_flag = ["vlPFC", "TEO", "all"]
        ...
        %behav = [1,7,8,18];%[4,5,17];% [7,8]%[5,7:10,21];%[4,5,7:10];%[4:8,17]; %[1:6,9:11,16,17]; %manually select behaviors of interest
        ...
        if length(unique(testlbls))==length(behav) && length(unique(trainlbls))==length(behav)
            if all(block1_behav(behav,2)>20) && all(block2_behav(behav,2)>20)
                ...
                mean_hitrate_baseline{s}(temp, chan) = mean(hitrate_baseline)
...
save('SVM_results_crossBlock.mat', "mean_hitrate_baseline","mean_hitrate","sd_hitrate","mean_hitrate_shuffled","behav","a_sessions","h_sessions","behav_categ")
