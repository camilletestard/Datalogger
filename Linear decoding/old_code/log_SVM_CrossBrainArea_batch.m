...
%         behavior_labels(behavior_labels==find(behav_categ=="Proximity"))=length(behav_categ); %Make proximity equal to rest
        ...
        for iter = 1:num_iter
            ...
            traindata = Spike_count_raster_final(:,strcmp(brain_label, "TEO")); trainlbls = behavior_labels_final;
            testdata = Spike_count_raster_final(:,strcmp(brain_label, "vlPFC")); testlbls = trainlbls;
            traindata = traindata(:,randsample(size(traindata,2), 110,'false')); testdata = testdata(:,randsample(size(testdata,2), 110,'false'));
