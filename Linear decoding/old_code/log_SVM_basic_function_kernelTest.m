function [hitrate, C] = log_SVM_basic_function(Input_matrix, Labels, kfolds, ldaflag, Normalize, kernel)
...
for fold = 1:kfolds
    ...
    disp(['-t,' num2str(kernel) ', -q'])
    model = svmtrain(trainlbls, traindata, ['-t,' num2str(kernel) ', -q']); %train the model using a linear kernel (-t: 0) or a RBF kernel (-t: 2) and default parameters
