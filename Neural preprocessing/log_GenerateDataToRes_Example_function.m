function [Spike_rasters, SpikeData, labels, labels_partner, behav_categ, block_times, monkey, reciprocal_set, social_set, ME_final, unit_count, groom_labels_all, brain_label] = log_GenerateDataToRes_function(filePath, temp_resolution, channel_flag, is_mac, with_NC, isolatedOnly)
...
% 1. Neural data matrix, size [#neurons x Time (to chosen resolution)] --> 'Spike_rasters' output matrix
% 2. Label vector which describes the behavior at time t [Time (to chosen
% resolution) x 11] --> 'labels' cell structure
...
% 3. Label for partner behavior, same columns as subject labels  --> 'labels_partner' cell structure
% 4. Label for grooming, length of the session, with the following columns:
%  --> 'groom_labels_all' matrix
...
% Other outputs:
%   SpikeData: Structure containing the spike times for each cell for each
%   channel (there can be multiple units per electrode or channel).
%   behav_categ: name of the behavioral categories observed (eg:1=aggression)
%   block_times: start and end times of experimental blocks 
%   monkey: name of monkey
%   reciprocal_set: set of behaviors which are "reciprocal" (i.e. one can
%   100% predict the behavior of the partner monkey based on that of the
%   subject)
%   social_set: set of behaviors that are social (with a conspecific, as
%   opposed to non-social)
%   ME_final: disregard for now
%   unit_count: # units for TEO, vlPFC and both (respectively)
%   brain_labels: To which brain region each neuron belongs to.
