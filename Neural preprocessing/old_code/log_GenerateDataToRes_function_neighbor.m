function [Spike_rasters, labels, labels_partner, labels_neighbor, behav_categ, block_times, monkey, reciprocal_set, social_set, ME_final, unit_count, groom_labels_all] = log_GenerateDataToRes_function(filePath, temp_resolution, channel_flag, is_mac, with_NC, isolatedOnly, smooth, sigma)
...
behavior_log_neighbor = readtable('EVENTLOG_restructured_neighbor.csv');% for neighbor
...
behavior_log_neighbor{:,'start_time_round'}=round(behavior_log_neighbor{:,'start_time'}*temp_resolution);
behavior_log_neighbor{:,'end_time_round'}=round(behavior_log_neighbor{:,'end_time'}*temp_resolution);
behavior_log_neighbor{:,'duration_round'}=behavior_log_neighbor{:,'end_time_round'}-behavior_log_neighbor{:,'start_time_round'};
...
idx_neighbor = find(behavior_log_neighbor{:,'duration_s'}<min_length);
behavior_log_neighbor{idx_neighbor,'start_time_round'} = 0; behavior_log_neighbor{idx_neighbor,'end_time_round'} = 0;
...
behavior_log_neighbor(end-2:end,:) = [];
...
behav_categ = ["Aggression","Proximity","Groom Give", "HIP","Foraging", "Vocalization","SS", "Masturbating",...
    ...
    "Head Bobbing", "Swinging", "Object manipulation","Mounting","watch.neighbor"];
...
%For neighbor monkey
start_times_neighbor = behavior_log_neighbor{:,'start_time_round'};
end_times_neighbor = behavior_log_neighbor{:,'end_time_round'};
Intervals_neighbor = [start_times_neighbor end_times_neighbor];
...
%%%%%% Create labels vector for NEIGHBOR monkey %%%%%%
labels_neighbor = cell(length_recording,10); %initialize dataframe
for s = 1:length_recording %for all secs in a session
    % this finds the index of the rows(2) that have x in between
    idx = find(s >= Intervals_neighbor(:,1) & s < Intervals_neighbor(:,2)); %find if this second belong to any interval
    %IMPORTANT note: interval exclude upper boundary as is.
    if ~isempty(idx) %if it belongs to an interval
        labels_neighbor{s,1} = behavior_log_neighbor{idx,'Behavior'}; %add behavior label in [plain english]
        labels_neighbor{s,2} = find(matches(behav_categ,labels_neighbor{s,1})); %add behavior label in [number]
        if length(labels_neighbor{s,2})>1 %If one behavior co-occurs with another behavior
            if ~isempty(setdiff(labels_neighbor{s,2}, double_behav_set)) %If behavior co-occurs with proximity or RR
                labels_neighbor{s,4} = 'co-occur with prox or RR';
                labels_neighbor{s,5} = 2;
                labels_neighbor{s,3} = setdiff(labels_neighbor{s,2}, double_behav_set); % only consider the other behavior (it that takes precedence over proximity and RR)
            else %If proximity & RR co-occur
                labels_neighbor{s,3}=find(matches(behav_categ,'Proximity')); % prioritize proximity
                labels_neighbor{s,4} = 'prox & RR co-occur';
                labels_neighbor{s,5} = 3;
            end
        else %If only one behavior happens in that sec
            labels_neighbor{s,3} = labels_neighbor{s,2};
            labels_neighbor{s,4} = 'single';
            labels_neighbor{s,5} = 1;
        end
        if length(labels_neighbor{s,3})~=1 %If two behaviors are co-occurring which do not include proximity or RR
            if any(labels_neighbor{s,3}==grmpr) % if one of the behavior includes groom present
                labels_neighbor{s,3}=grmpr; %Keep groom present
                labels_neighbor{s,4} = 'grmpr co-occur';
                labels_neighbor{s,5} = 4;
            elseif any(labels_neighbor{s,3}==omv) % if one of the behavior includes other monkey vocalize
                labels_neighbor{s,3}=omv; %Keep OMV
                labels_neighbor{s,4} = 'omv co-occur';
                labels_neighbor{s,5} = 5;
            else %Otherwise just choose the second behavior for now..
                %                 error('More than one behavior simultansouly, not with proximity or RR')
                %                 return
                labels_neighbor{s,3}= labels_neighbor{s,3}(2); %2nd behavior (HIP/HIS take precedence over aggression)
                labels_neighbor{s,4} = 'Other key behav co-occur';
                labels_neighbor{s,5} = 3;
            end
        end
    else %if not
        labels_neighbor{s,1} = NaN; labels_neighbor{s,2} = length(behav_categ); labels_neighbor{s,3} = length(behav_categ); labels_neighbor{s,5} = 0;%Set behavior category to "NaN" and label to rest
    end
    %Add behavior information: reciprocal behavior?
    if any(reciprocal_set == labels_neighbor{s,3}) %if behavior is reciprocal
        labels_neighbor{s,6} = "reciprocal";
        labels_neighbor{s,7} = 1;
    else
        labels_neighbor{s,6} = "non-reciprocal";
        labels_neighbor{s,7} = 0;
    end
    %Add behavior information: social vs. non-social
    if any(social_set == labels_neighbor{s,3})
        labels_neighbor{s,8} = "social"; %if behavior is social
        labels_neighbor{s,9} = 1;
    else
        labels_neighbor{s,8} = "non-social";
        labels_neighbor{s,9} = 0;
    end
    %Add block information
    if s<=block_times{1,'end_time_round'}
        labels_neighbor{s,10} = string(block_times{1,'Behavior'});
        labels_neighbor{s,11} = 1;
    elseif s>block_times{1,'end_time_round'} && s<=block_times{2,'end_time_round'}
        labels_neighbor{s,10} = string(block_times{2,'Behavior'});
        labels_neighbor{s,11} = 2;
    elseif s>block_times{2,'end_time_round'}
        labels_neighbor{s,10} = string(block_times{3,'Behavior'});
        labels_neighbor{s,11} = 3;
    end
    %
end
