...
%% Dim reduce the multidimensional vector
n_units=size(Spike_rasters,1);
for t = 2:length_recording-1
    idx_fire{t} = find(Spike_rasters(randi(size(Spike_rasters,1),1,n_units),t)==1);
    num_units_firing(t) = length(idx_fire{t});
end
figure; hist(num_units_firing, 10); ylabel('Number of bins'); xlabel('Number units firing')
length(find(num_units_firing>1))/length_recording %2+
length(find(num_units_firing>2))/length_recording %3+
length(find(num_units_firing>3))/length_recording %4+
length(find(num_units_firing>4))/length_recording %5+
length(find(num_units_firing>5))/length_recording %6+
max(num_units_firing)
