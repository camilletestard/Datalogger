function [Spike_rasters, labels, labels_partner, behav_categ, block_times, monkey, reciprocal_set, social_set, ME_final, unit_count] = log_GenerateDataToRes_function(filePath, temp_resolution, channel_flag, is_mac, with_NC, isolatedOnly)
...
if isolatedOnly==1
    load(['Neural_data_' session '_IsolatedUnits.mat'])
else
...
for s = 1:length_recording %for all secs in a session
    ...
    if ~isempty(idx) %if yes
        ...
        labels_partner{s,1} = NaN; labels_partner{s,2} = length(behav_categ); labels_partner{s,3} = length(behav_categ); labels{s,5} = 0;%Set behavior category to "NaN" and label to rest
    end
