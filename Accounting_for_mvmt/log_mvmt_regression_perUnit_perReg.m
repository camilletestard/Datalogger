...
for s =15%session_range %1:length(sessions)
    ...
    for sig = 3:length(sigma_list)
        ...
        for unit = 1:size(Y_final ,2) %for now one unit at a time.
            ...
            Adj_rsq_all(unit) = mdl_all.Rsquared.Adjusted; %extract adjusted Rsq
            if Adj_rsq_all(unit)>0.8 %these units are not biological and should be removed from the dataset
                units_to_remove = [units_to_remove unit];
                Adj_rsq_all(unit)=nan;
            ...
            for pred = 1:size(X_all, 2)-1
                ...
                Full_rsq_perReg(unit, pred) = mdl.Rsquared.Adjusted; %extract adjusted Rsq
                ...
                Unq_rsq_perReg(unit, pred) = Adj_rsq_all(unit) - mdl.Rsquared.Adjusted; %extract adjusted Rsq
    ...
    [~, idx_sorted]=sort(nanmean(Full_rsq_perReg(TEO_units,:)));
    boxplot([Full_rsq_perReg(TEO_units,idx_sorted),Adj_rsq_all(TEO_units)])
    ylabel('Full Rsq')
    ...
    [~, idx_sorted]=sort(nanmean(Unq_rsq_perReg(TEO_units,:)));
    boxplot([Unq_rsq_perReg(TEO_units,idx_sorted)])
    ylabel('Unique Rsq')
    ...
    %vlPFC
    [~, idx_sorted]=sort(nanmean(Full_rsq_perReg(vlPFC_units,:)));
    subplot(2,2,3); hold on
    boxplot([Full_rsq_perReg(vlPFC_units,idx_sorted),Adj_rsq_all(vlPFC_units)])
    ylabel('Full Rsq')
    xticks([1:11]); xlim([0.5 11.5])
    xticklabels(pred_labels([idx_sorted,length(pred_labels)]))
    ax = gca;
    ...
    [~, idx_sorted]=sort(nanmean(Unq_rsq_perReg(vlPFC_units,:)));
    subplot(2,2,4); hold on
    boxplot([Unq_rsq_perReg(vlPFC_units,idx_sorted)])
    ylabel('Unique Rsq')
    xticks([1:10]); xlim([0.5 10.5])
    xticklabels(pred_labels(idx_sorted))
    ax = gca;
